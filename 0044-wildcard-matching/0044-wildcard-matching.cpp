...
public:
    ...
    bool solveMem1(string &str, string &pattern, int i, int j, vector<vector<int>> &dp){
        ...
        if(str[i-1] == pattern[j-1] || pattern[j-1] == '?'){
            return dp[i][j] = solveMem1(str, pattern, i-1, j-1, dp);
        }
        else if(pattern[j-1] == '*'){
            return dp[i][j] = (solveMem1(str, pattern, i-1, j, dp) || solveMem1(str, pattern, i, j-1, dp));
        }
        else{
            return false;
        }
    ...
    bool spaceOptimize(string str, string pattern){
    ...
    bool isMatch(string s, string p) {
        ...
        // return solveMem1(s, p, s.length(), p.length(), dp);

        // Bottom up aproach
        // return solveTab(s, p);

        //spaceOptimize
        return spaceOptimize(s,p);
    }
};